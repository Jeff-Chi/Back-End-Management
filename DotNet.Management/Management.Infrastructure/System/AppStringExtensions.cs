...
namespace Management.Infrastructure
{
    public static class AppStringExtensions
    {
        ...
        /// <summary>
        /// Indicates whether this string has value.
        /// </summary>
        ...
        public static string ToSentenceCase(this string str, bool useCurrentCulture = false)
        {
            ...
            return useCurrentCulture
                ...
                : Regex.Replace(str, "[a-z][A-Z]", m => m.Value[0] + " " + char.ToLowerInvariant(m.Value[1])).ToLower();
        }
        ...
        public static string ToSnakeCase(this string str, bool useCurrentCulture = false)
        {
            ...
            return useCurrentCulture
                ...
                : Regex.Replace(str, "[a-z][A-Z]", m => m.Value[0] + "_" + char.ToLowerInvariant(m.Value[1])).ToLower();
        }
        ...
        public static string ToPascalCase(this string str, char splitChar, bool useCurrentCulture = false)
        {
            if (splitChar != '_' && splitChar != '-' && splitChar != ' ')
            {
                return ToPascalCase(str, useCurrentCulture);
            }
            str = str.Trim(splitChar);
            if (string.IsNullOrWhiteSpace(str))
            {
                return str;
            }

            string pattern = splitChar == ' ' ? "\\s" : splitChar.ToString();

            return useCurrentCulture
                ? Regex.Replace(str, $"{pattern}[a-zA-Z]", m => char.ToUpper(m.Value[1]).ToString()).ToPascalCase(useCurrentCulture)
                : Regex.Replace(str, $"{pattern}[a-zA-Z]", m => char.ToUpper(m.Value[1]).ToString()).ToPascalCase(useCurrentCulture);
        }

        public static string ToCamelCase(this string str, char splitChar, bool useCurrentCulture = false)
        {
            return ToPascalCase(str, splitChar, useCurrentCulture).ToCamelCase(useCurrentCulture);
        }
            ...
            bool useRegex)
        {
            ...
            {
                var match = matchCase ?
                    Regex.Match(input, text) :
                    Regex.Match(input, text, RegexOptions.IgnoreCase);
                if (match.Success)
                {
                    index = match.Index;
            ...
            else
            {
                index = matchCase ?
                    input.IndexOf(text) :
                    input.IndexOf(text, StringComparison.CurrentCultureIgnoreCase);
                if (index > -1)
                {
                    length = text.Length;
                }
            ...
            if (index > -1)
            {
                ...
                if (matchWholeWord)
                {
                    ...
                    if (result && index + length < input.Length)
                    {
                        // check the char after the match
                        if (char.IsLetterOrDigit(input[index + length]))
                        {
                            result = false;
                        }
                    }
        ...
        public static List<string> ReadAllLines(this string str)
        {
